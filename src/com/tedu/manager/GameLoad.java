...
//			以后用的 都是 xml 和 json
			pro.clear();
			pro.load(new InputStreamReader(maps, StandardCharsets.UTF_8));
...
//				就可以自动的创建和加载 我们的地图啦 
	...
	public static void loadImg() {
		...
		pro.clear();
		try {
			pro.load(new InputStreamReader(texts, StandardCharsets.UTF_8));
			Set<Object> set = pro.keySet();
			for(Object o:set) {
				String urlString = pro.getProperty(o.toString());
				java.net.URL url = classLoader.getResource("com/tedu/" + urlString);
				if (url == null) {
	...
	public static void loadPlay() {
		// loadObj(); // Already called in GameThread
